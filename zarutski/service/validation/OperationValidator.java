...
public class OperationValidator {
    ...
    private static boolean validateCardTransfer(TransferData transferData) {
        ...
        String accNumber = transferData.getSenderAccNumber(); // номер счёта
        String senderCardNumber = transferData.getSenderCardNumber();
        String destinationNumber = transferData.getDestinationNumber(); // карта получателя счёта
        String confirmationCode = transferData.getConfirmationCode();

        if (senderCardNumber.equals(destinationNumber)) {
            return false;
        }
